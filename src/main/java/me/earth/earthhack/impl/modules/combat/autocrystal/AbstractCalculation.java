        ...
        extends Finishable implements Globals
{
    ...
    protected final List<PlayerEntity> raw;

    protected double maxY = Double.MAX_VALUE;
    protected List<PlayerEntity> friends;
    protected List<PlayerEntity> enemies;
    protected List<PlayerEntity> players;
    protected List<PlayerEntity> all;
    ...
    protected void setFriendsAndEnemies()
    {
        ...
        List<List<PlayerEntity>> split = CollectionUtil.split(raw,
            p -> p == null
                || EntityUtil.isDead(p)
                || p.equals(mc.player)
        ...
        shieldRange += enemies.stream().peek(e -> all.add(e)).filter(e -> e.squaredDistanceTo(mc.player) <= MathUtil.square(module.shieldRange.getValue())).count();
        shieldRange += players.stream().peek(e -> all.add(e)).filter(e -> e.squaredDistanceTo(mc.player) <= MathUtil.square(module.shieldRange.getValue())).count();
            ...
            for (Entity entity : entities)
            {
                if (entity instanceof EndCrystalEntity
                        ...
                        && entity.getBoundingBox()
                                 .intersects(new Box(forcePos.up())))
                {
    ...
    protected boolean place(PlaceData data)
    {
        ...
        for (AntiTotemData antiTotemData : data.getAntiTotem())
        {
                    ...
                    && !antiTotemData.isBlocked())
            {
                ...
                Entity entity = new EndCrystalEntity(
                    mc.world,
                    pos.getX() + 0.5f,
                    pos.getY() + 1,
                    pos.getZ() + 0.5f);
                ...
                if (god)
                {
                            ...
                            antiTotemData.getCorresponding())
                    {
                        ...
                        PlayerEntity player = antiTotemData.getFirstTarget();
        ...
        if (antiTotem != null)
        {
            PlayerEntity player = antiTotem.getFirstTarget();
                ...
                && module.forceTimer.passed(module.attempts.getValue()))
        {
            ...
            for (Map.Entry<PlayerEntity, ForceData> entry :
    ...
    protected boolean placeCheckPre(BlockPos pos)
    {
        ...
        if ((module.placeRangeCenter.getValue()
            ? pos.getSquaredDistanceFromCenter(x, y, z)
            : pos.getSquaredDistance(x, y, z)) >= MathUtil.square(module.placeRange.getValue()))
        {
            return false;
        }
        ...
        if (pos.getSquaredDistanceFromCenter(x, y, z)
                                                          ...
                                                          boolean rotating)
    {
        ...
        if (absorb)
        {
            ...
            Direction absorbFacing = module.liquidHelper.getAbsorbFacing(
