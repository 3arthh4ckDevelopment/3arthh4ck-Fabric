...
public class NoRender extends Module
{
    ...
    public boolean isValidArmorPiece(EquipmentSlot slot)
    {
        if (!this.isEnabled()) return true;
        if (slot == EquipmentSlot.HEAD && helmet.getValue()) return false;
        if (slot == EquipmentSlot.CHEST && chestplate.getValue()) return false;
        if (slot == EquipmentSlot.LEGS && leggings.getValue()) return false;
        return slot != EquipmentSlot.FEET || !boots.getValue();
    }
