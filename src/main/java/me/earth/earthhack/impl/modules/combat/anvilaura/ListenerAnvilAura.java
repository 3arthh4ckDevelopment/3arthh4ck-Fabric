...
{
    private static final ModuleCache<AntiSurround> ANTI_SURROUND =
    ...
    {
        if (ANTI_SURROUND.returnIfPresent(AntiSurround::isActive, false))
        {
            return;
        }
    ...
    protected TargetResult getTargets(TargetResult result)
    {
        ...
        switch (mode) {
            case Render -> {
                List<Box> renderBBs = new ArrayList<>(
    ...
    {
        return Objects.requireNonNullElse(disableString, "Disabled, no Anvils.");
    }
    ...
    private boolean doTrap(AnvilResult anvilResult)
    {
                ...
                && module.crystalTimer.passed(module.crystalDelay.getValue()))
        {
            ...
            for (EndCrystalEntity entity : mc.world.getEntitiesByClass(
                    EndCrystalEntity.class, new Box(upUp), e -> true))
            {
            ...
            for (Vec3i vec3i : CRYSTAL_OFFSETS)
            {
                ...
                if (BlockUtil.canPlaceCrystal(pos, false, false))
                {
                    ...
                    BlockHitResult result = RayTraceUtil.getBlockHitResult(
                    ...
                    module.placeBlock(
                        pos, result.getSide(), module.rotations, result.getPos());
                             ...
                             TargetResult result)
    {
        ...
        if (highest != null)
        {
            ...
            BlockState mineState = HELPER.getBlockState(highest);
            module.mineBB = mineState
                            .getCollisionShape(mc.world, highest)
                            .getBoundingBox()
    ...
    private boolean placeTop(AnvilResult anvilResult, TargetResult result)
    {
        ...
        for (Direction facing : Direction.HORIZONTAL)
        {
            ...
            if (!HELPER.getBlockState(pos).isReplaceable())
            {
