...
final class HoleESPData extends DefaultData<HoleESP>
{
    public HoleESPData(HoleESP module)
    {
        ...
        register(module.mode, """
                - Polling : Recalculates holes around you every tick.
                - Invalidation : Experimental way of calculating holes: Calculates all holes in a chunk first,
                then recalculates small areas around every block that's being modified. Probably more
                performant than Polling, but can't solve the bottleneck of rendering many holes.
                Note that changing this Setting's value will require you to rejoin the server.""");
