...
public class HelperObby implements Globals
{
    ...
    {
        double maxY = 0;
        List<PlayerEntity> filteredPlayers = new LinkedList<>();
        for (PlayerEntity player : players)
        {
            if (player == null
                || EntityUtil.isDead(player)
                || player.getY() > mc.player.getY() + 18
                || player.squaredDistanceTo(mc.player)
        ...
        if (fastObby != 0)
        {
            ...
            else
            {
                ...
                for (PlayerEntity player : filteredPlayers)
                {
        ...
        for (PositionData positionData : obbyData.values())
        {
            ...
            {
                for (PlayerEntity p : filteredPlayers)
                {
