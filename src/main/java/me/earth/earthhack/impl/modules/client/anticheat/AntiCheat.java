...
public class AntiCheat extends Module {
    ...
    protected final Setting<Boolean> newVerEntities =
        register(new BooleanSetting("Under-Entities", false));
    ...
    public AntiCheat() {
        ...
        data.register(oldVer, "Use on servers that are still on Minecraft" +
            " version 1.12 or lower. Takes into account that crystals can't " +
        ...
        syncNow.addObserver(e -> {
            if (e.getValue()) {
                ...
                oldVer.notifyObservers(oldVer.getValue());
        ...
        addIterateObserver(oldVer, (e, s) -> {
            if (oldVer.getName().equals(s.getName())
                    && s.getInitial() instanceof Boolean) {
                set(s, e.getValue());
            }
        });
            ...
            if (acRotate != null) {
                if (e.getValue()) {
                    switch (acRotate.getValue()) {
                        case Break, None -> set(acRotate, e.getValue()
                                ? ACRotate.Break : ACRotate.None);
                        default -> set(acRotate, e.getValue()
