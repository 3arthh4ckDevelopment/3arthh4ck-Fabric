...
//                e -> handleBlockChangePacket(e, e.getPacket().getChangedBlocks().length)));
        ...
        {
            ((IChunk) event.chunk()).setHoleVersion(((IChunk) event.chunk()).getHoleVersion() + 1);
            if (config.isUsingInvalidationHoleManager() && mc.world != null)
            {
                holes.entrySet().removeIf(e ->
                {
                    if (!mc.world.isChunkLoaded(ChunkSectionPos.getSectionCoord(e.getKey().getX()), ChunkSectionPos.getSectionCoord(e.getKey().getY())))
                    {
