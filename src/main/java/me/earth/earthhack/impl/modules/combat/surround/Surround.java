...
public class Surround extends ObbyModule
{
    ...
    protected void center()
    {
            ...
            && (holeC.getValue() || !HoleUtil.isHole(startPos, false)[0]))
        {
            ...
            double y = centerY.getValue() ? startPos.getY() : getPlayer().getY();
            double z = startPos.getZ() + 0.5;
            getPlayer().setPosition(x, y, z);
            getPlayer().setVelocity(0.0, getPlayer().getVelocity().getY(), 0.0);
        }
        else
        {
            setPosition = true;
        ...
        switch (movement.getValue())
        {
            ...
            case Y:
                ...
                teleported = startPos == null
                    ? 0.0
                    : currentPos.getSquaredDistance(startPos);
    ...
    {
        if (pos.getX() >= mc.world.getWorldBorder().asVoxelShape().getBoundingBox().maxX || pos.getZ() >= mc.world.getWorldBorder().asVoxelShape().getBoundingBox().maxZ)
            return false;
    ...
    protected boolean isBlockingTrap(BlockPos pos, List<PlayerEntity> players)
    {
        ...
        Direction relative = getFacingRelativeToPlayer(pos, getPlayer());
        if (relative != null
            && !trapExtend.getValue()
        ...
        for (PlayerEntity player : players)
        {
            if (player == null
                ...
                || player.squaredDistanceTo(pos.toCenterPos()) > 9)
            {
                continue;
            }
    ...
    protected Direction getFacingRelativeToPlayer(BlockPos pos,
                                                   PlayerEntity player)
    {
                                           ...
                                           List<PlayerEntity> players)
    {
        ...
        if (noTrapBlock.getValue())
        {
            ...
            for (BlockPos trap : trapBlocks)
            {
                ...
                {
                    Direction r = getFacingRelativeToPlayer(trap, getPlayer());
                    if (r != null)
                    {
    ...
    public PlayerEntity getPlayerForRotations()
    {
        if (FREECAM.isEnabled())
        {
            PlayerEntity target = FREECAM.get().getPlayer();
            if (target != null)
            {
                return target;
            }
