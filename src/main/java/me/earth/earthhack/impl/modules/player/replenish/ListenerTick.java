...
final class ListenerTick extends ModuleListener<Replenish, TickEvent>
{
    ...
    private int findSlot(ItemStack current, int count)
    {
        ...
        int maxDiff  = current.getMaxCount() - count;
        int minSize  = current.getMaxCount() > module.minSize.getValue()
        ...
        if (InventoryUtil.canStack(current, mc.player.getInventory().getStack(mc.player.getInventory().selectedSlot)))
        {
            return -2;
        }
