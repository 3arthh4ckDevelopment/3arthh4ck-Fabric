    ...
    public static void start(Surround module, MotionUpdateEvent event)
    {
        ...
        if (module.updatePosAndBlocks())
        {
            ...
            if (!hasPlaced || !module.crystalCheck.getValue())
            {
                ...
                if (module.getPlayer().getVelocity().getX() != 0.0
                        || module.getPlayer().getVelocity().getZ() != 0.0)
                {
                    // place in front of us first to stop our movement.
                    // I am aware that this looks really bad
                    BlockPos pos = new BlockPos(module.getPlayer().getBlockPos())
                            .add((int) (module.getPlayer().getVelocity().getX() * 10000),
                                 0,
                                    (int) (module.getPlayer().getVelocity().getY() * 10000));

                    surrounding.sort(Comparator.comparingDouble(
                            p -> p.getSquaredDistance(
                                    pos.getX() + 0.5,
                                    pos.getY(),
                                    pos.getZ() + 0.5)));
                }
