...
public class AutoCrystal extends Module
{
    ...
    {
        int bind = holdFacePlaceKey.getValue().getKey(); //TODO: fix, use the key event and backport to 1.12.2
    ...
    protected void reset()
    {
        ...
        bypassPos = null;
        post.clear();
        mc.execute(crystalRender::clear);
                ...
                && multiThread.getValue())
        {
            ...
            else
            {
                ...
                if (closest != null
                        ...
                        && BlockUtil.canBeFeetPlaced(closest, true, newVer.getValue())
                        && earlyFeetThread.getValue()
                        && Managers.TICK.valid(Managers.TICK.getTickTimeAdjusted(), 0, maxEarlyThread.getValue()))
                {
    ...
    public boolean isNotCheckingRotations() {
        return noPacketFlyRotationChecks.getValue()
                && (/*PingBypass.PACKET_SERVICE.isPacketFlying()*/ false
    ...
    public boolean isEating() {
        ...
        return mc.player.getActiveHand() != null
                && !stack.isEmpty()
    ...
    public boolean isEatingOffhand() {
        ItemStack stack = mc.player.getActiveItem();
        return mc.player.getActiveHand().equals(Hand.OFF_HAND)
                && !stack.isEmpty()
                && stack.getItem().getUseAction(stack) == UseAction.EAT;
    }
