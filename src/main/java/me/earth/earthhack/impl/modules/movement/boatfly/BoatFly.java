...
public class BoatFly extends Module
{
    ...
    protected void sendPackets(Entity riding)
    {
        ...
        if (invalid.getValue() && invalidTickCount++ >= invalidTicks.getValue())
        {
            ...
            if (boatInvalid.getValue() && mc.player.isRiding())
            {
                Vec3d vec = invalidMode.getValue().createOutOfBounds(mc.player.getVehicle().getPos(), 1337);
                riding.setPos(vec.getX(), vec.getY(), vec.getZ());
                VehicleMoveC2SPacket packet = new VehicleMoveC2SPacket(riding);
