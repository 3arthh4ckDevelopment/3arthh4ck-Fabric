...
public class Phase extends BlockESPModule implements CollisionEvent.Listener
{
    ...
    protected void onEnable()
    {
        ...
        {
            double yawCos =
                    Math.cos(Math.toRadians(player.yaw + 90.0f));
            double yawSin =
                    Math.sin(Math.toRadians(player.yaw + 90.0f));
            player.setPosition(
                    player.getX() + 1.0 * blocks.getValue() * yawCos
                            + 0.0 * blocks.getValue() * yawSin,
                    player.getY(),
                    player.getZ() + (1.0 * blocks.getValue() * yawSin
