...
final class ListenerObby extends ObbyListener<HoleFiller>
{
    ...
    public void invoke(MotionUpdateEvent event)
    {
        ...
        if (module.requireTarget.getValue())
        {
            ...
            if (module.target == null
                || module.target.squaredDistanceTo(mc.player)
    ...
    protected TargetResult getTargets(TargetResult result)
    {
        ...
        {
            PlayerEntity p = RotationUtil.getRotationPlayer();
            Vec3d v = p.getPos()
                       .add(p.getVelocity().getX(), p.getVelocity().getY(), p.getVelocity().getZ());

            targets.removeIf(pos -> pos.getSquaredDistance(v.x, v.y, v.z)
