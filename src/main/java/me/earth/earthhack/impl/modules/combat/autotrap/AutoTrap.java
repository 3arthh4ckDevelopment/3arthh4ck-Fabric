...
{
    /** Directions for the top helpingPos */
    private static final Direction[] TOP_FACINGS = new Direction[]
    {
            Direction.UP,
            Direction.NORTH,
            Direction.WEST,
    ...
    protected final Map<PlayerEntity, Double> speeds = new HashMap<>();
    /** Caches trapping positions for players while looking for a target */
    protected final Map<PlayerEntity, List<BlockPos>> cached = new HashMap<>();
    ...
    @Override
    protected boolean shouldHelp(Direction facing, BlockPos pos)
    {
    ...
    protected TargetResult getTargets(TargetResult result)
    {
        cached.clear();
        updateSpeed();
        PlayerEntity newTarget = calcTarget();
    ...
    public boolean isValid(PlayerEntity player)
    {
        ...
        {
            if (player.squaredDistanceTo(mc.player) <= 36
    ...
    {
        PlayerEntity player = spot.getModel().getPlayer();
    ...
    private void updateSpeed()
    {
        for (PlayerEntity player : Managers.ENTITIES.getPlayers())
        {
            ...
            {
                double xDist = player.getX() - player.prevX;
                double yDist = player.getY() - player.prevY;
                double zDist = player.getZ() - player.prevZ;
     ...
     */
    private double getSpeed(PlayerEntity player)
    {
    ...
    private List<BlockPos> getPositions(PlayerEntity player)
    {
        ...
        BlockPos playerPos = new BlockPos(new Vec3i((int) player.getX(), (int) player.getY(), (int) player.getZ()));
    ...
    private BlockPos[] findTopHelping(List<BlockPos> positions, boolean first)
    {
        ...
        for (BlockPos pos : positions)
        {
            ...
            for (Direction facing : TOP_FACINGS)
            {
                ...
                Direction helpingFace = BlockUtil.getFacing(helping, HELPER.getClientWorld(), false);
