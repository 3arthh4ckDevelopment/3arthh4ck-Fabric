...
final class ListenerMotion extends ModuleListener<Timer, MotionUpdateEvent>
{
    ...
    public void invoke(MotionUpdateEvent event)
    {
        if (event.getStage() == Stage.PRE)
        {
            ...
            {
                if (event.getPitch() - ((IClientPlayerEntity) mc.player).earthhack$getLastReportedPitch() == 0.0
                     && event.getYaw() - ((IClientPlayerEntity) mc.player).earthhack$getLastReportedYaw() == 0.0)
                {
                    offset = -offset;
