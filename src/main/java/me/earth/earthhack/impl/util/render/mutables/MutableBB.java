...
import me.earth.earthhack.impl.util.math.VectorUtil;
...
public class MutableBB extends Box implements BB
{
    /**
    ...
    /**
     * Helper method that returns a new {@link Box} with the given value for {@link #maxY} and all other
     * values taken from this bounding box.
     */
    public Box setMaxY(double y2)
    {
    ...
    public boolean equals(Object p_equals_1_)
    {
        ...
        {
            BB Box = (BB)p_equals_1_;

            if (Double.compare(Box.getMinX(), this.getMinX()) != 0)
            {
                return false;
            }
            else if (Double.compare(Box.getMinY(), this.getMinY()) != 0)
            {
                return false;
            }
            else if (Double.compare(Box.getMinZ(), this.getMinZ()) != 0)
            {
                return false;
            }
            else if (Double.compare(Box.getMaxX(), this.getMaxX()) != 0)
            {
                return false;
            }
            else if (Double.compare(Box.getMaxY(), this.getMaxY()) != 0)
            {
                return false;
            }
            else
            {
                return Double.compare(Box.getMaxZ(), this.getMaxZ()) == 0;
            }
        }
    }
    ...
    /**
     * Creates a new {@link Box} that has been contracted by the given amount, with positive changes
     ...
     * <tr><td><pre><code>new Box(0, 0, 0, 4, 4, 4).contract(2, 2, 2)</code></pre></td><td><pre><samp>box[0.0,
     * 0.0, 0.0 -> 2.0, 2.0, 2.0]</samp></pre></td></tr>
     * <tr><td><pre><code>new Box(0, 0, 0, 4, 4, 4).contract(-2, -2, -
     * 2)</code></pre></td><td><pre><samp>box[2.0, 2.0, 2.0 -> 4.0, 4.0, 4.0]</samp></pre></td></tr>
     * <tr><td><pre><code>new Box(5, 5, 5, 7, 7, 7).contract(0, 1, -
     * 1)</code></pre></td><td><pre><samp>box[5.0, 5.0, 6.0 -> 7.0, 6.0, 7.0]</samp></pre></td></tr>
     * <tr><td><pre><code>new Box(-2, -2, -2, 2, 2, 2).contract(4, -4,
    ...
    /**
     * Creates a new {@link Box} that has been expanded by the given amount, with positive changes increasing
     * max values and negative changes decreasing min values.
     *
     * <h3>Samples:</h3>
     * <table>
     * <tr><th>Input</th><th>Result</th></tr>
     * <tr><td><pre><code>new Box(0, 0, 0, 1, 1, 1).expand(2, 2, 2)</code></pre></td><td><pre><samp>box[0, 0,
     * 0 -> 3, 3, 3]</samp></pre></td><td>
     * <tr><td><pre><code>new Box(0, 0, 0, 1, 1, 1).expand(-2, -2, -2)</code></pre></td><td><pre><samp>box[-2,
     * -2, -2 -> 1, 1, 1]</samp></pre></td><td>
     * <tr><td><pre><code>new Box(5, 5, 5, 7, 7, 7).expand(0, 1, -1)</code></pre></td><td><pre><samp>box[5, 5,
    ...
    /**
     * Creates a new {@link Box} that has been contracted by the given amount in both directions. Negative
     ...
     * <table>
     * <tr><th>Input</th><th>Result</th></tr>
     * <tr><td><pre><code>new Box(0, 0, 0, 1, 1, 1).grow(2, 2, 2)</code></pre></td><td><pre><samp>box[-2.0, -
     * 2.0, -2.0 -> 3.0, 3.0, 3.0]</samp></pre></td></tr>
     * <tr><td><pre><code>new Box(0, 0, 0, 6, 6, 6).grow(-2, -2, -2)</code></pre></td><td><pre><samp>box[2.0,
     * 2.0, 2.0 -> 4.0, 4.0, 4.0]</samp></pre></td></tr>
     * <tr><td><pre><code>new Box(5, 5, 5, 7, 7, 7).grow(0, 1, -1)</code></pre></td><td><pre><samp>box[5.0,
     * 4.0, 6.0 -> 7.0, 8.0, 6.0]</samp></pre></td></tr>
     * <tr><td><pre><code>new Box(1, 1, 1, 3, 3, 3).grow(-4, -2, -3)</code></pre></td><td><pre><samp>box[-1.0,
    ...
    /**
     * Creates a new {@link Box} that is expanded by the given value in all directions. Equivalent to {@link
    ...
    {
        return new Box(this.minX + (double)pos.getX(), this.minY + (double)pos.getY(), this.minZ + (double)pos.getZ(), this.maxX + (double)pos.getX(), this.maxY + (double)pos.getY(), this.maxZ + (double)pos.getZ());
    }
    ...
    @Nullable
    public BlockHitResult calculateIntercept(Vec3d vecA, Vec3d vecB)
    {
        ...
        return vec3d == null ? null : new BlockHitResult(vec3d, direction, BlockPos.ofFloored(vec3d.x, vec3d.y, vec3d.z), true);
    }

    @VisibleForTesting
    boolean isClosest(Vec3d p_186661_1_, @Nullable Vec3d p_186661_2_, Vec3d p_186661_3_)
    {
        return p_186661_2_ == null || p_186661_1_.squaredDistanceTo(p_186661_3_) < p_186661_1_.squaredDistanceTo(p_186661_2_);
    }
    ...
    {
        Vec3d vec3d = VectorUtil.getIntermediateWithXValue(p_186671_4_, p_186671_3_, p_186671_1_);
    ...
    {
        Vec3d vec3d = VectorUtil.getIntermediateWithYValue(p_186663_4_, p_186663_3_, p_186663_1_);
    ...
    {
        Vec3d vec3d = VectorUtil.getIntermediateWithZValue(p_186665_4_, p_186665_3_, p_186665_1_);
