        ...
        implements AttackingModule
{
    ...
    static
    {
        Bus.EVENT_BUS.register(new EventListener<>
    ...
    public boolean execute()
    {
        ...
        if (!packets.isEmpty())
        {
            if (attacking != null)
            {
                ...
                mc.player.networkHandler.sendPacket(attacking);
    ...
    public boolean placeBlock(BlockPos pos)
    {
            ...
            && (!rayTraceBypass.getValue() || Visible.INSTANCE.check(pos)))
        {
                        ...
                        || smartRay.getValue() == RayTraceMode.Force))
            {
                ...
                for (Direction facing : Direction.values())
                {
                    ...
                    BlockState state = HELPER.getBlockState(helpingPos);
        ...
        Direction initialFacing = BlockUtil.getFacing(pos, HELPER.getClientWorld());
        if (shouldHelp(initialFacing, pos))
        {
            ...
            for (Direction facing : Direction.values())
            {
                ...
                Direction helpingFacing = BlockUtil.getFacing(helpingPos,
                                                               HELPER.getClientWorld());
    ...
    public void setState(BlockPos pos)
    {
        ...
        {
            Item item = slot == -2 ? mc.player.getOffHandStack()
                                              .getItem()
                                   : mc.player.getInventory()
                                              .getStack(slot)
                                              .getItem();
            if (item instanceof BlockItem) // this should always be the case!
            {
