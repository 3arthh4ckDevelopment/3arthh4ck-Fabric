...
public class PathFinder implements Globals
{
    ...
    private static final Direction[] SN = {Direction.SOUTH, Direction.NORTH};
    private static final Direction[] EW = {Direction.EAST, Direction.WEST};
    private static final Direction[] UD = {Direction.UP, Direction.DOWN};
                                    ...
                                    TriPredicate<BlockPos, Pathable, Entity> c)
    {
        ...
        if (pathable.getFrom().squaredDistanceTo(new Vec3d(pos.getX(), pos.getY(), pos.getZ())) > MathUtil.square(pr)
        ...
        for (int i = 0; i < CHECKS.length; i++)
        {
            ...
            Direction[] facings = produceOffsets(check[0],
                                                  check[1],
                                                  xDiff,
                                                  yDiff,
                                                  zDiff);
    ...
    private static boolean check(BlockPos pos,
                                 ...
                                 TriPredicate<BlockPos, Pathable, Entity> c)
    {
        return pathable.getFrom().squaredDistanceTo(new Vec3d(pos.getX(), pos.getY(), pos.getZ())) > MathUtil.square(pr)
                || ignored.contains(pos)
    ...
    public static Direction[] produceOffsets(boolean yFirst,
                                              boolean xFirst,
                                              int xDiff,
                                              int yDiff,
                                              int zDiff)
    {
        Direction[] result = new Direction[Math.abs(xDiff)
                                           + Math.abs(yDiff)
                                           + Math.abs(zDiff)];
        int index = 0;
        if (yFirst)
    ...
    private static int apply(Direction[] array,
                             int diff,
                             int start,
                             Direction[] facings)
    {
